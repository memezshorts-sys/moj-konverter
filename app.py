...
st.markdown("""
    ...
    /* 1. STIL ZA SIDEBAR (Crveni krug) */
    [data-testid="stSidebar"] {
        background-color: #1e1e2f !important;
        border-right: 1px solid rgba(255, 255, 255, 0.1);
    }
    
    /* 2. STIL ZA UNOSNA POLJA U SIDEBARU (Zeleni krug) */
...
# Primjer elemenata u sidebaru da vidiš promjenu
with st.sidebar:
    st.image("https://flaticon.com", width=100)
    st.subheader("Postavke")
    st.text_input("Naziv tvrtke", placeholder="Unesite naziv...")
    st.selectbox("Model plaćanja", ["HR99", "HR00", "HR01"])
        ...
        if iban_match:
            ...
            for offset in range(-3, 4):
                if 0 <= i + offset < len(lines):
                    ...
                    if date_match: datum = date_match.group(0)
...
if uploaded_file:
    try:
        ...
        if data:
            ...
            last_date = data[-1]["Datum"] if data else datetime.now().strftime('%d.%m.%Y')
            
            if "0,40" in raw_text:
                data.append({"Datum": last_date, "Konto": "4650", "Naziv": "Naknada banke", "IBAN": "", "Duguje": "0.40", "Potražuje": "0.00"})
                ukupno += 0.40
            
            data.append({"Datum": last_date, "Konto": "1000", "Naziv": "Izvod", "IBAN": "", "Duguje": "0.00", "Potražuje": "{:.2f}".format(ukupno)})
